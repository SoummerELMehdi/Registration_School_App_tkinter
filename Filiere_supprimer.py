...
def supprimer():
    ...
    if(Filiere_id.get()==""):
        mbox.showinfo("Entrer Id Filiere !")
    else:
        conn=sqlite3.connect('App.db')
        c=conn.cursor()
        c.execute("DELETE FROM Filiere WHERE idFiliere=?",(Filiere_id.get(),))
        conn.commit()

        mbox.showinfo("Delete Status", "fILIERE supprimé")
        conn.close()

modifier = Tk()
modifier.geometry("368x130")
modifier.title ( "Supprimer Filiere" )
